...
function colIndexToLetter(i) { return String.fromCharCode(65 + i); }
function norm(v) { return String(v ?? "").trim(); }
function digitsOnly(v) { return norm(v).replace(/\D/g, ""); }
function extractKeyFromName(name) { const s = norm(name); if (!s) return ""; return s.split("_", 1)[0]; }

function indexVideosById(videos) {
  const m = new Map();
  for (const v of videos || []) { if (!v?.id) continue; m.set(norm(v.id), v); m.set(digitsOnly(v.id), v); }
  return m;
}
...
function decideNextStatus({ mode, currentStatus, dub }) {
  ...
  if (mode === "dub") { if (dub?.dubbingId) { if (cur === STATUS.NOT_UPLOADED) return STATUS.NEEDS_REVISION; return null; } return null; }
  if (mode === "export") { const isExported = !!dub?.downloadUrl || dub?.status === "exported"; if (isExported && cur !== STATUS.EXPORTED) return STATUS.EXPORTED; return null; }
  return null;
}

async function auth() {
  if (!SPREADSHEET_ID) { console.error("Set SPREADSHEET_ID"); process.exit(1); }
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    try {
      const auth = new google.auth.GoogleAuth({ scopes: ["https://www.googleapis.com/auth/spreadsheets"], keyFilename: process.env.GOOGLE_APPLICATION_CREDENTIALS });
      const client = await auth.getClient();
      console.log("[AUTH] GOOGLE_APPLICATION_CREDENTIALS");
      return google.sheets({ version: "v4", auth: client });
    } catch { console.warn("[AUTH] GOOGLE_APPLICATION_CREDENTIALS failed, falling back"); }
  }
  if (process.env.GCP_SA_KEY_JSON) {
    try {
      const sa = JSON.parse(process.env.GCP_SA_KEY_JSON);
      let key = sa.private_key || ""; if (key.includes("\\n")) key = key.replace(/\\n/g, "\n");
      if (!sa.client_email || !key) throw new Error("invalid");
      const jwt = new google.auth.JWT(sa.client_email, null, key, ["https://www.googleapis.com/auth/spreadsheets"]);
      await jwt.authorize();
      console.log("[AUTH] GCP_SA_KEY_JSON");
      return google.sheets({ version: "v4", auth: jwt });
    } catch { console.warn("[AUTH] GCP_SA_KEY_JSON failed, falling back"); }
  }
  const raw = await fs.readFile(SA_KEY_PATH, "utf8");
  const sa = JSON.parse(raw);
  let key = sa.private_key || ""; if (key.includes("\\n")) key = key.replace(/\\n/g, "\n");
  if (!sa.client_email || !key) { throw new Error("Invalid service account JSON"); }
  const jwt = new google.auth.JWT(sa.client_email, null, key, ["https://www.googleapis.com/auth/spreadsheets"]);
  await jwt.authorize();
...
async function getHeaderMap(sheets) {
  const { data } = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${SHEET_TITLE}!1:1`, majorDimension: "ROWS" });
  const headers = (data.values && data.values[0]) || [];
  const map = new Map(); headers.forEach((h, i) => map.set(norm(h), i));
  const missing = []; if (!map.has(norm(KEY_HEADER))) missing.push(KEY_HEADER);
  const hasAnyCore = CORE_HEADERS.some(h => map.has(norm(h))); if (!hasAnyCore) missing.push(...CORE_HEADERS);
  if (missing.length) { throw new Error(`Sheet header eksik: ${missing.join(", ")}`); }
  return { map, headers };
}
...
async function getBodyRows(sheets, colCount) {
  ...
  const { data } = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range, majorDimension: "ROWS" });
...
async function readJson(path) {
  ...
  if (data && typeof data === "object") {
    for (const k of ["items","data","results","records","dubbings","videos"]) {
...
(async () => {
  try {
    ...
    const colIdx = {
      id: headerMap.get(norm("ElevenLabs Dubbing ID")),
      name: headerMap.get(norm("ElevenLabs Dubbing Name")),
      url: headerMap.get(norm("Dubbed Video URL")),
    ...
    const cell = (idx, rowNumber) => `${SHEET_TITLE}!${colIndexToLetter(idx)}${rowNumber}:${colIndexToLetter(idx)}${rowNumber}`;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (Number.isInteger(colIdx.id)) batch.push({ range: cell(colIdx.id, rowNumber), values: [[dub.dubbingId || ""]] });
      ...
      if (Number.isInteger(colIdx.url) && dub.downloadUrl) {
        batch.push({ range: cell(colIdx.url, rowNumber), values: [[dub.downloadUrl]] });
      }
      ...
      if (Number.isInteger(colIdx.status)) {
        ...
        batch.push({ range: cell(colIdx.status, rowNumber), values: [[value]] });
      }
